...
| ACTIVE | Annulation du titulaire | Aucun CHECKOUT déjà autorisé ou incertain; invalider atomiquement toute attente QR | CANCELLED | Enregistrer cancelledAt et recalculer la disponibilité |
| ACTIVE | reservedUntil atteint | Aucun retrait confirmé et aucune opération CHECKOUT déjà autorisée ou incertaine; terminer une éventuelle attente QR | EXPIRED | Enregistrer expiredAt et recalculer la disponibilité |
...
- Tant qu’un CHECKOUT déjà autorisé non terminal ou physiquement incertain existe, l’expiration de la réservation est différée afin d’éviter une course entre le minuteur et la preuve physique.
...
| ACTIVE | RETURN autorisé | Défi du titulaire consommé, locker disponible, horaire valide, aucune autre opération de retour active | RETURN_PENDING | Enregistrer returnRequestedAt et autoriser la LockerOperation RETURN préparée |
...
sequenceDiagram
    ...
    M->>A: Valider le QR du retour préparé
    A->>O: Consommer le défi et autoriser RETURN
...
stateDiagram-v2
    ...
    AWAITING_LOCAL_PROOF --> AUTHORIZED : QR consommé et règles revalidées
...
| AWAITING_LOCAL_PROOF | Demande admissible et défi créé | Aucune commande de serrure; attente du scan local |
| AUTHORIZED | Défi affiché, valide et consommé; identité, accès, horaire, réservation ou prêt, readiness et absence de conflit validés | Le backend autorise une commande limitée dans le temps |
...
|---|---|---|---|---|
| REQUESTED | Préparation acceptée | Préconditions satisfaites | AWAITING_LOCAL_PROOF | Défi créé et instruction d’écran durable |
| AWAITING_LOCAL_PROOF | QR soumis par l’initiateur | Défi affiché, valide, lié à cette opération; toutes les gardes revalidées | AUTHORIZED | localProofValidatedAt = authorizedAt; expiresAt = authorizedAt + 120 s |
...
| AWAITING_LOCAL_PROOF | Défi expiré, y compris fermeture ou fin de réservation | EXPIRED | Aucune ouverture et aucun changement de prêt |
| AWAITING_LOCAL_PROOF | Annulation, 5 secrets erronés, redémarrage, perte de connexion ou garde devenue invalide | FAILED | Défi INVALIDATED; écran effacé; aucune commande |
...
### 5.7 Machine du défi local

```mermaid
stateDiagram-v2
    direction TB
    [*] --> PENDING
    PENDING --> CONSUMED : QR valide et autorisation atomique
    PENDING --> EXPIRED : échéance atteinte
    PENDING --> INVALIDATED : annulation ou refus définitif
    CONSUMED --> [*]
    EXPIRED --> [*]
    INVALIDATED --> [*]
```

L’accusé `ACCESS_CHALLENGE_DISPLAYED` renseigne `displayedAt` sans faire évoluer la `LockerOperation`. Il est distinct de `COMMAND_ACKNOWLEDGED`, réservé à la serrure. Les terminaux du défi sont irréversibles.

Une préparation RETURN maintient le prêt `ACTIVE`; seul `AWAITING_LOCAL_PROOF → AUTHORIZED` peut le passer à `RETURN_PENDING`. Un prêt déjà en récupération conserve `RETURN_PENDING`. Si la réservation expire pendant l’attente QR, les deux expirent atomiquement; cette attente ne justifie aucun report de réservation. L’annulation avant autorisation invalide le défi et termine l’opération sans ouvrir.
...
| Préparation CHECKOUT | ACTIVE | AWAITING_LOCAL_PROOF | Aucun prêt | — |
| Préparation RETURN | FULFILLED | AWAITING_LOCAL_PROOF | ACTIVE, ou RETURN_PENDING en récupération | — |
| RETURN autorisé après QR | FULFILLED | AUTHORIZED | RETURN_PENDING | — |
...
- Une réservation distante ne crée aucune opération ni commande de serrure.
- La préparation crée seulement un défi; le prêt reste inchangé.
- Un défi expiré, rejoué, non affiché, d’un autre compte ou d’une autre opération ne permet aucune ouverture.
- Deux validations simultanées créent exactement une commande.
- Une nouvelle session du hub invalide l’ancien défi.
- L’expiration d’une réservation en attente QR n’est pas différée.
